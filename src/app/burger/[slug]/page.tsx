...
export default async function BurgerDetail({ params }: Params) {
  ...
  const listJson: unknown = listRes.ok
    ? await listRes.json().catch(() => ({ items: [] as unknown[] }))
    : ({ items: [] as unknown[] } as const);
  type ApiProduct = {
    id?: string;
    _id?: string;
    name?: string;
  ...
  const priceStr = Number.isFinite(priceNum)
    ...
    : '€0,00';
  const image =
    typeof chosen!.imageUrl === 'string' && /^https?:\/\//.test(chosen!.imageUrl as string)
      ? (chosen!.imageUrl as string)
      : 'https://images.unsplash.com/photo-1550547660-d9450f859349?q=80&w=1200&auto=format&fit=crop';

  const product: Product = {
    id: Number.isFinite(numericFromSlug) ? numericFromSlug : 1,
    title: String(chosen!.name || titlePart.replace(/-/g, ' ')).replace(/\b\w/g, (c) =>
      c.toUpperCase()
    ),
    price: priceStr,
    image,
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-[420px_1fr] gap-6">
        ...
        <div className="card p-5">
          ...
          <p className="mt-2 text-sm text-white/80 max-w-prose">
            Lorem ipsum dolor sit amet consectetur. Eros quis sed gravida volutpat dis mattis
            congue. Dictum id elementum eget commodo in nibh. Sit nullam aliquam in viverra commodo
            eget amet in faucibus pretium. Lobortis tempor nunc condimentum id varius metus accumsan
            at. Lectus tristique purus cubilia nibh commodo suscipit tortor.
          </p>
