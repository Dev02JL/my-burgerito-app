...
export default function ProfilePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const data: { items?: Array<{ id: string; createdAt: string; total: number }> } =
          await res.json();
        ...
        const withItems: OrderWithItems[] = await Promise.all(
          list.map(async (o) => {
            ...
            const itemsData: { items?: PopulatedItem[] } = itemsRes.ok
              ? await itemsRes.json()
              : { items: [] };
            return {
              id: o.id,
  ...
  return (
    <main className="container-page py-8">
      <Link
        href="/"
        className="mb-6 inline-flex items-center gap-3 text-sm text-white/70 hover:text-white/90 transition-colors"
      >
