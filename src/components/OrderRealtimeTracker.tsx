...
export default function OrderRealtimeTracker({ orderId }: Props): JSX.Element {
    ...
    <div className="container-page py-10" role="region" aria-live="polite" aria-atomic="true">
			...
			<p className="mt-1 text-white/70 text-sm">Commande #{orderId} — {connected ? "en temps réel" : "déconnecté"}</p>
			...
			{done ? (
				<div className="mt-8">
					<Link href="/commande/livree" className="h-10 rounded-md px-4 btn-accent inline-flex items-center">Voir le récapitulatif</Link>
				</div>
			) : null}
		</div>
	);
